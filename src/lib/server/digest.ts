/**
 * Notis-digest: händelser under ett tyst-fönster → EN läsbar push.
 * Ren modul (ingen DB) → testbar. Speglas i supabase/functions/_shared/digest.ts.
 *
 * Principer:
 *  - Rubriken bär poängen ("William handlade 6 varor"), brödtexten detaljerna.
 *  - Ångrat nettas bort per sak: bockad + avbockad = inget, skapad + borttagen = inget.
 *  - Flera slags saker → "Nytt från William" med en rad per grupp.
 *  - Klick öppnar fliken för den viktigaste gruppen.
 */
